...
export async function login (email: string, password: string) {
  const { data: { accessToken } } = await api.post('/auth/login', { email, password })
  saveAuthToken(accessToken)
  return getSavedAuthToken()
}

export async function updateToken () {
  const { data: { accessToken } } = await api.post('/auth/update-token')
  saveAuthToken(accessToken)
}
...
export async function sync (timeout?: number) {
  ...
  await Promise.all(data.transactions.map(async transaction => {
